...
function ReqTopic(props) {
    ...
    return (
        <Modal show={show} onHide={handleShow} className='pop-up'>
            ...
            <button onClick={handleShow} className='close-req-but'>X</button>
            </ModalHeader>
            <ModalBody className='req-box'>
            <form onSubmit={handleReq}>
                <input type='text' className='input-req' onChange={newTopic}></input>
                <input type='submit' value='SEND REQUEST' className='send-req-but' onClick={handleShow}></input>
            </form>
