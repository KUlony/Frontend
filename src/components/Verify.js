...
function Verify(probs) {
  ...
  const [work2, setWork2] = useState(true);
  ...
  return (
    <div className="verify_container">
      ...
      {!isForgot && (
        <p className="verify_resend" onClick={resend_register}>
          RESEND CODE
        </p>
      )}
      {isForgot && (
        <p className="verify_resend" onClick={resend_change}>
          RESEND CODE
        </p>
      )}

      {!isForgot && (
        <button className="verify_button" onClick={verify_register}>
          VERIFY
        </button>
      )}
      {isForgot && (
        <button className="verify_button" onClick={verify_change}>
          VERIFY
        </button>
      )}
      ...
      <div className={`register_verify  ${display5 ? 'none' : null}`}>
        <Change
          display_ch={display_change}
          display_ve={display_ve}
