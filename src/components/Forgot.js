...
function Forgot(probs) {
  ...
  return (
    <div className="forgot_container">
      <img className="forgot_xmark" src={xmark_img} alt="xmark_img" onClick={display_fg}></img>
      <h2 className="forgot_header">Forgot password?</h2>
      <h3 className="forgot_subheader">Please enter your e-mail address</h3>
      <input className="forgot_input" type="email" placeholder="EMAIL" value={email} onChange={(e)=>{setEmail(e.target.value)}}></input>
      
      <div className="forgot_noti">
            {error && <div className="forgot_error">{error}</div>}
      </div>
      <button className="forgot_button" onClick={forgot}>CONFIRM</button>
    
      <div className={`register_verify  ${display2? 'none':null }`}>
        <Verify display_ve={display_verify} email={email} isForgot={true} display_fg={display_fg}/>
      </div>
    </div>
  );
}
