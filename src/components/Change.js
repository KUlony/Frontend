...
function Change(probs) {
  ...
  return (
    <div className="change_container">
      ...
      <input className="change_input" type="password" placeholder="NEW PASSWORD" value={password} onChange={(e)=>{setPassword(e.target.value)}}></input>

      <h3 className="change_subheader">Confirm password</h3>
      <input className="change_input" type="password" placeholder="CONFIRM PASSWORD" value={confirmpassword} onChange={(e)=>{setConfirmpassword(e.target.value)}}></input>
      
      <div className="change_noti">
            {error && <div className="change_error">{error}</div>}
      </div>
      <button className="change_button" onClick={confirm}>CONFIRM</button>
    </div>
  );
}
